class Repository			
	...
	def initialize name, params = {}
		INITIALIZATION_SYNC.synchronize do		
			...
			@dir = params[:directory] || CONFIG[:directory]; @dir.should_not! :be_nil						
			...
			@stream_storage = StreamStorage::Storage.new @name, @dir, CONFIG[:buffer_size]																							
			...
			index_def << Indexes::HashIndex.new(:path){|e| e.entity_path}
	...
	def by_id om_id		
		@entity_loading_sync.synchronize do
			entity = @entities_cache[om_id]							
			unless entity
				entity = AnEntity::EntityType.load om_id, self, @storage
				entity.should_not! :be_nil
				@entities_cache[om_id] = entity
			end			
			return entity
		end
	end
	
	def om_id_include? om_id
		@entity_loading_sync.synchronize do			
			entity = @entities_cache[om_id]
			if entity
				return true
			else
				return AnEntity::EntityType.storage_include? om_id, @storage
			end			
		end
	end
	...
	def include? path
		...
		om_id = index(:path).get_om_id path
