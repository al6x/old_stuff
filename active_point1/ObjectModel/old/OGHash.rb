...
#		end
		...
		def og_save copy, storage, transaction
			storage[:hashes].filter(:entity_id => copy.entity_id).delete
			copy._hash.each do |k, v|					
				...
				if proc
					...
					assert(v).is_a [ValueObject, Entity]
					key_data, key_klass = v.entity_id, Engine::Marshal::COMPLEX_OBJECT
				end
				
				storage[:hashes].insert(:entity_id => copy.entity_id, 
		...
		def og_delete object, storage
			storage[:hashes].filter(:entity_id => object.entity_id).delete
		end
		
		def og_load hash, entity_id, storage, o_resolver
			rows = storage[:hashes].filter :entity_id => entity_id
			rows.each do |row|								
