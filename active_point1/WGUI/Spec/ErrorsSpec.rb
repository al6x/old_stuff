...
#				start_webserver; join_webserver;
		...
		describe "WGUI Redirect" do
			...
			it "Should disable links that points to the same page" do
				go 'localhost:8080/?t=Should disable links that points to the same page&portlet=c/d'
			...
			it "Should correct display link with hash (a => b) state" do
				go 'localhost:8080/?t=Should correct display link with hash (a => b) state&portlet=n/v'
			...
			it "Should redirect from incorrect URI to correct one" do
				go 'localhost:8080/?t=Should redirect from incorrect URI to correct one&portlet=name/value/invalid'
				wait_for{uri !~ /invalid/}
				
				go 'localhost:8080/?t=Should redirect from incorrect URI to correct one&portlet=invalid'
				wait_for{uri !~ /invalid/}
			end
			
			it "Should correct display link with path (/a/b/c) state" do
				go 'localhost:8080/?t=Should correct display link with path (/a/b/c) state&portlet=/somepath'
			...
			it "Should always correct setup state, even if its empty" do
				go 'localhost:8080/?t=Should always correct setup state, even if its empty'
				wait_for.should have_text("'/'")
				go 'localhost:8080/?t=Should always correct setup state, even if its empty&portlet=/a/b'
				wait_for.should have_text("'/a/b'")
			end
			
			it "Error when build an Link with empty operator attribute" do
				go 'localhost:8080/?t=Error when build an Link with empty operator attribute'
			end
			
			it "build builds ordinary wigets but doesn't build links" do
				go "localhost:8080/?t=build builds ordinary wigets but doesn't build links"
