	...
	def build
		unless multiple			
			if values.size < simple_editor_limit
				@content = WSelect.new(@values, @value).set(:modify => @modify)
			else
			...
			@value ||= []
			if values.size < simple_editor_limit									
				@content = WMultiselect.new(@values, @value).set(:modify => @modify)
			else				
...
#	end
	...
	def simple_editor_limit
		@simple_editor_limit ||= self.class["config.yaml"][:simple_editor_limit]
	end
