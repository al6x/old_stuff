Paperclip::ClassMethods.class_eval do
  ...
  def validates_maximum_file_size attachment
    ...
    define_method method_name do
      max_size = Models::Account.current? ? Models::Account.current.max_file_size : config.max_file_size(1.megabyte)
  ...
  def validates_maximum_user_files_size attachment
    ...
    define_method method_name do
      ...
      max_files_size = Models::Space.current.max_user_files_size        
      return if max_files_size == 0
      
      files_size = Models::User.current.files_size      
      size = file_size_for(attachment)
      old_size = old_file_size_for(attachment)
      if size + files_size > max_files_size - old_size
        errors.add(attachment, t(:maximum_user_files_size_exceeded, 
  ...
  def validates_maximum_account_files_size attachment
    ...
    define_method method_name do
      ...
      files_size = Models::Account.current.files_size
      max_files_size = Models::Account.current.max_account_files_size        
      size = file_size_for(attachment)
      old_size = old_file_size_for(attachment)
      if size + files_size > max_files_size - old_size
        errors.add(attachment, t(:maximum_account_files_size_exceeded, 
          max_files_size: (max_files_size / 1000), 
          files_size: (files_size / 1000)
        )) 
      end
