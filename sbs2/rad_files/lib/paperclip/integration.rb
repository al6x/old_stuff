...
Paperclip::ClassMethods.class_eval do  
  def has_attached_file name, options = {}
    ...
    validates_each name, logic: lambda{|model|
...
Paperclip::Attachment.class_eval do
  ...
  # - can't accept hashes from rack
  def assign uploaded_file
    if uploaded_file.is_a? Hash
      rack_hash = uploaded_file
    ...
    instance_write(:file_name,       (rack_hash[:filename] || uploaded_file.original_filename).strip.gsub(/[^A-Za-z\d\.\-_ ]+/, '_'))
    instance_write(:content_type,    ((rack_hash[:type] && Mime[File.extname(rack_hash[:type])]) || uploaded_file.content_type).to_s.strip)
