...
module Functional
  class ValidatesLengthOfTest < Test::Unit::TestCase
    test "given short value, when validated, then error is in the objects error collection" do
      ...
      instance = klass.new
      instance.valid?
      assert_equal ["is invalid"], instance.errors[:name]
    end
    ...
    test "given in constraint, when validated, then error is in the objects error collection" do
      klass = Class.new do
        include Validatable
        attr_accessor :name
        validates_length_of :name, :in => 2..4
      end
    ...
    test "given in constraint, when validated, then valid is true" do
      klass = Class.new do
        include Validatable
        attr_accessor :name
        validates_length_of :name, :in => 2..4
