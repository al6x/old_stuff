...
Expectations do
  ...
  expect false do
    validation = Validatable::ValidatesTrueFor.new stub_everything, :name, :logic => lambda {|_self| false }
    validation.valid?(stub_everything)
  end

  expect true do
    validation = Validatable::ValidatesTrueFor.new stub_everything, :name, :logic => lambda {|_self| true }
    validation.valid?(stub_everything)
  end
