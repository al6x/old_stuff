...
Expectations do
  ...
  expect true do
    validation = Validatable::ValidatesLengthOf.new stub_everything, :username, :in => 2..4
  ...
  expect true do
    validation = Validatable::ValidatesLengthOf.new stub_everything, :username, :in => 2..4, :allow_nil => true
    instance = stub(:username => nil)
    validation.valid?(instance)
  end

  expect true do
    validation = Validatable::ValidatesLengthOf.new stub_everything, :username, :in => 2..4, :allow_blank => true
    instance = stub(:username => '')
    validation.valid?(instance)
  end

  # expect true do
  #   options = [:message, :if, :times, :level, :groups, :maximum, :minimum, :is, :in, :allow_nil]
