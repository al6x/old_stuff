...
wgui.progress_stop = function() {
	...
	// window.uidriver_page_available = true; <== WARNING Can't do this, because 
	// there are also AJAX redirect. And it will not be working if we do this.
}
...
wgui.copy_inputs_to_form = function(inputs, form) {	
    ...
    for (var i = 0; i < inputs.length; i++) {
        ...
        for (var j = 0; j < childs.length; j++) {
            ...
            if (child.type == 'text' || child.type == 'password' || child.type == 'textarea') {
			...
			if (child.type == 'select-multiple') {								
				var text = ""; //child.cloneNode(true);
				var count = 0;
				for (var k = 0; k < child.options.length; k++) {
					if (count > 0) text = text + "\n"
					if (child.options[k].selected){
						count = count + 1
						text = text + child.options[k].value;	
					}					
				}
				
				// Hack for processing empty selection (Brouser doesn't returns anything if selection empty)
				var input = window.document.createElement('input');
				input.name = child.name
				input.value = text
				/*empty_option.text = "__empty_selection__";
				empty_option.value = "__empty_selection__";
				empty_option.selected = true;
				clone.options[clone.options.length] = empty_option;*/
