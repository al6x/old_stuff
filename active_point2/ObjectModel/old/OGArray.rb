...
#		end
		...
		def og_save copy, storage, transaction
			storage[:arrays].filter(:name => copy.name).delete
			copy._array.each_with_index do |v, index|		
				...
				storage[:arrays].insert(:name => copy.name, 
																:index => index.to_s, 
		...
		def og_delete object, storage
			storage[:arrays].filter(:name => object.name).delete
		end
		
		def og_load array, name, storage, o_resolver
			rows = storage[:arrays].filter :name => name
			rows.each do |row|								
