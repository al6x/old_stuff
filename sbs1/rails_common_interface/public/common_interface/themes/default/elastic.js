...
(function($){
	...
	window.Elastic = function Elastic(context){
		...
		egreg = /(^|\s+)don\-(\d+)(\s+|$)/,
		esreg = /(^|\s+)dspan\-(\d+)(\s+|$)/,
		eareg = /(^|\s+)dadaptive\-(\d+)\-(\d+)(\s+|$)/;

		eg   = [];
		egcl = egnl = 0;

		econs = $.find('.dcolumns', context);
		for(i = 0, il = econs.length; i < il; i++){
			...
			else{                                                       egml = $.find('> .dcolumn, > .dcontainer > .dcolumn', econ).length; }
			econ  = $.find('> .dcontainer', econ)[0] || econ;
			econw = width(econ);
			ecrw  = econw / egml;
			ecw   = Math.round( ecrw );
			
			if(econclass.indexOf('dadaptive-') > -1 && eareg.test(econclass)){
			...
			for(j = 0, jl = ecols.length; j < jl; j++){
				...
				if(egnl == egml || j == (jl - 1) || ecolclass.indexOf('dfinal') > -1){ eg.push(ecol); egcl = 0;     rp = 1; }
				...
				if(rp){
					for(k = 0, kl = eg.length; k < kl; k++){
						...
						if(     ecclass.indexOf('dfixed')   > -1){ efcs.push(ec); efcsw += width(ec); }
						else if(ecclass.indexOf('delastic') > -1){ eecs.push(ec); }
						else                                    {
							ecs.push(ec); 
							if(ec == eg[kl - 1]){
					...
					else if(egnl < egml && ecolclass.indexOf('dfinal') > -1){
						ecol.style['margin' + ( (econclass.indexOf('dinverted') > -1) ? 'Left' : 'Right')] = (econw - ecsw - efcsw) + 'px';
					}
					eg = []; egnl = 0;
				}
	...
	Elastic.reset = function Elastic_reset(context){
		...
		h = $.find('.dsame-height > *, .dfull-height, .delastic-height', context);
		for(i = 0, hl = h.length; i < hl; i++){h[i].style.height = '';}
		p = $.find('.dvertical-center, .dcenter, .bottom', context);
		for(i = 0, pl = p.length; i < pl; i++){p[i].parentNode.style.paddingTop = ''; p[i].parentNode.style.height = '';}
		w = $.find('.dcolumn:not(.dfixed), .full-width', context);
		for(i = 0, wl = w.length; i < wl; i++){w[i].style.width = '';}
		m = $.find('.dcolumn.dfinal', context);
	...
	Elastic.helpers = {
		...
		'dsame-height'      : function Elastic_helper_sameHeight(context){
			$('.dsame-height', context).each(function(){
				var columns = $('> *', this);
				var maxHeight = 0;
		...
		'dfull-height'      : function Elastic_helper_fullHeight(context){
			$('.dfull-height', context).each(function(){
				var _this = $(this);
				_this.css('height', $(this.parentNode).height() - ( _this.outerHeight(true) - _this.height() ));
			});
		},
		'delastic-height'   : function Elastic_helper_elasticHeight(context){
			$('.delastic-height', context).each(function(){
				var _this = $(this);
				var h = 0;
				$('> *:not(.delastic-height)', this.parentNode).each(function(){
		...
		'dcenter'           : function Elastic_helper_center(context){
			$('.dvertical-center, .dcenter', context).each(function(){
