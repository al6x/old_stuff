class BrigeServlet
	...
	def sync_get path, params, uri_sid, uri, req	
		begin
			cookies_sid = req.cookies[SESSION_ID]						
			sid = uri_sid || cookies_sid
			...
			MicroContainer::ScopeManager.activate_thread sid do
				BrigeServlet.notify_observers :get
	...
	def sync_post path, req, iframe, sid		
		begin
			...
			MicroContainer::ScopeManager.activate_thread sid do
				BrigeServlet.notify_observers :post
				
				executor = window.visit(Visitors::FindById.new(executor_id)).result
				unless executor
					err_msg = "Page has been expired! (Invalid executor ID '#{executor_id}'!)"
					err_msg += " Or maybe you forgot to add Button to :children." if $debug
					raise MicroContainer::InvalidSessionError, err_msg 
				end
				...
				if BrigeServlet.exception_wrapper
					BrigeServlet.exception_wrapper.call lambda{executor.execute action_name}
				else
...
<textarea>#{{:error => e.message}.to_json}</textarea>
	...
	def sync_push sid		
		begin
			...
			MicroContainer::ScopeManager.process_async_observers_for_session sid do
				BrigeServlet.notify_observers :push
	...
	def async_resource path, req, uri_sid
		begin			 
			sid = uri_sid || req.cookies[SESSION_ID]
			raise MicroContainer::InvalidSessionError, "Page has been expired! (Invalid Session ID '#{sid}'!)" unless valid_sid? sid			
	...
	class << self
		inherit Observable
		attr_accessor :exception_wrapper		
	end
end
