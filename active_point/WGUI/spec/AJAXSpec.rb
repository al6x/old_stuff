...
#start_webserver; join_webserver;
	describe "AJAX" do
		it 'Should correct works when response is empty' do
			go 'localhost:8080/ui?t=Should correct works when response is empty'
			click 'Empty responce'
			sleep 0.5
		end
		
		it "Should correct works with exception" do
			#			Temporary disabled. There is no currently ability to check UIDRiver for alerts.
			#			go 'localhost:8080'
			#			click 'Failed'
		...
		it "Should refresh only Components that where marked with refresh method but not others" do		
			go 'localhost:8080/ui?t=Should refresh only Components that where marked with refresh method but not others'
		...
		it "Should refresh all Childs of refreshed Component" do		
			go 'localhost:8080/ui?t=Should refresh all Childs of refreshed Component'
			wait_for.should have_text('One')
			wait_for.should have_text('Two')
			wait_for.should have_text('Three')
			click 'Refresh'
		...
		it "Should get input values only from Input Components inside specified Container but not others" do		
			go 'localhost:8080/ui?t=Should get input values only from Input Components inside specified Container but not others'
			type 'Value1' => 'value 1'
			type 'Value2' => 'value 2'
			type 'Value3' => 'value 3'
			click 'Get Values'		
...
#			clicked.should be_true
		...
		it "Should get input values only from specified Input Components but not others" do					
			go 'localhost:8080/ui?t=Should get input values only from specified Input Components but not others'
			type 'Value1' => 'value 1'
			type 'Value2' => 'value 2'
			type 'Value3' => 'value 3'
			click 'Get Values'		
...
#			clicked.should be_true
		...
		it "Should refresh only Components and Containers marked with refresh method but not others" do					
			go 'localhost:8080/ui?t=Should refresh only Components and Containers marked with refresh method but not others'
			click 'Get Values'
